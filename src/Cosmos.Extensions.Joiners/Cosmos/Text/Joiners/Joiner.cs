...
public partial class Joiner : IJoiner
{
    ...
    {
        return list.JoinToString(_on, JoinerUtils.GetObjectPredicate<T>(Options), converter, Options.GetReplacer<T>());
    }
    ...
    {
        return list.JoinToString(_on, JoinerUtils.GetIndexedObjectPredicate<T>(Options), converter, Options.GetIndexedReplacer<T>());
    }
