...
"; // 6 lines
    ...
    {
        StringLines.CountByLines(TestSampleOne).ShouldBe(6);
    }

    [Fact(DisplayName = "Split a text and returns the all lines test")]
    public void LineItemsTest()
    {
        var texts = StringLines.SplitByLines(TestSampleOne).ToList();
    ...
    {
        var val = StringLines.TruncateByLines(TestSampleOne, 3);
        var lines = StringLines.SplitByLines(val).ToList();
